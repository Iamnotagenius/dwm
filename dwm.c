...
void zoomfocused(const Arg *arg) {
...
bstack(Monitor *m) {
    ...
    for (i = 0, mx = m->gappx, tx = m->wx + m->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < m->nmaster) {
            ...
            resize(c, tx, ty + m->gappx, w - (2 * c->bw), h - (2 * c->bw), 0);
            if (tw != m->ww)
                tx += WIDTH(c) + m->gappx;
        }
    }
...
void arg_spawndefault(const Arg* arg)  {
